...
public class HmacParametersHandler extends AbstractSignHandler {
    ...
    public Result<?> verify(RequestSignInfo requestSignInfo) {
        ...
        Map<String, String> signParameters = Maps.newLinkedHashMapWithExpectedSize(sourceParameters.size());
        if (signAnnotation.includes().length != 0) {
            // 仅仅签名包含的
            for (String include : signAnnotation.includes()) {
                Optional.ofNullable(sourceParameters.get(include)).ifPresent(v -> signParameters.put(include, v));
            }
        } else {
            signParameters.putAll(sourceParameters);
        }
        // 去除签名key的参数
        signParameters.remove(signAnnotation.signKey());
        String signFromLocal = HmacUtil.generateHmac(signParameters, signedSecret);
        ...
        String log = String.format("【%s】：验签结果对不匹配，key：%s，请求：%s，本地：%s,签名的内容:%s",
                signAnnotation.title(), signedSecret, signFromRequest, signFromLocal, signParameters);
